...
#
#   1. PHOTO ROUTING     — a captioned fridge photo that names the household's
#                          CONFIGURED cooking voice decodes as a photo and elects
#                          that voice, exactly like a text message with the same
#                          caption.
...
#                          uncaptioned (unaddressed) photo names nobody, so it
#                          falls to whichever persona this household's
#                          `household.toml` gives `coordination` — and an
#                          unaddressed but FOOD-shaped caption falls to whichever
#                          persona it gives `cooking`. Both by CONFIGURED ROLE.
#   5. CONFIG, NOT NAME  — the same fixtures re-run against a roster that swaps
#                          `cooking` and `coordination` between the two personas:
#                          who answers follows the config, not the id. This is
...
# An uncaptioned group photo — names nobody at all.
cat >"$scratch/uncaptioned.json" <<'JSON'
{"update_id":1,"message":{"message_id":61,"date":1720000000,"chat":{"id":-1000000000001,"type":"supergroup"},"from":{"id":8905220378,"is_bot":false,"username":"housemate"},"photo":[{"file_id":"x","width":1280,"height":720,"file_size":90000}]}}
JSON

# An unaddressed photo whose CAPTION is food-shaped — names nobody, but its
# content belongs to the cooking side of the roster.
cat >"$scratch/food_caption.json" <<'JSON'
{"update_id":1,"message":{"message_id":63,"date":1720000000,"chat":{"id":-1000000000001,"type":"supergroup"},"from":{"id":8905220378,"is_bot":false,"username":"housemate"},"caption":"what do we still need for dinner?","photo":[{"file_id":"x","width":1280,"height":720,"file_size":90000}]}}
JSON
...
refute_grep() {
    ...
    if echo "$out" | grep -qi "$needle"; then
        loud_fail "$desc: '$needle' must not appear, got: $out"
    fi
}
...
echo "1. photo routing — captioned photo elects the configured cooking voice ($COOK):"
out="$(plan @update.json)"
expect_grep "is a photo" "$out" "^photo — "
expect_grep "largest frame chosen" "$out" "1 image"
expect_grep "routes to the configured cook" "$out" "routes to: $COOK"
...
refute_grep "must not re-add an item already on the list" "$out" "add 'lemons'"
# The family-facing reply must NOT contain the machine marker.
refute_grep "machine marker leaked into the family reply" "$out" "SHOPPING_UPDATE"
...
# gives `coordination` — the group's point of contact.
...
# An unaddressed but food-shaped caption belongs to the cooking side, so the
...
refute_grep "the previous coordinator must not still answer" "$swapped_out" "routes to: $COORD"
