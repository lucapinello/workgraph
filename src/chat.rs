...
/// Generate a request ID for a chat-producing client.
///
/// Format: `{namespace}-{unix_millis}-{uuid_v7}`. The explicit millisecond
/// component preserves the existing operator-friendly shape, while the
/// canonical UUID v7 suffix supplies collision resistance and strict
/// per-process creation ordering even when many requests are generated in the
/// same millisecond.
pub fn generate_request_id(namespace: &str) -> String {
    let uuid = uuid::Uuid::now_v7();
    let timestamp = uuid
        .get_timestamp()
        .expect("a UUID v7 generated by now_v7 always carries a timestamp");
    let (seconds, nanos) = timestamp.to_unix();
    let millis = seconds
        .saturating_mul(1_000)
        .saturating_add(u64::from(nanos / 1_000_000));
    format!("{namespace}-{millis}-{uuid}")
}
...
mod tests {
    ...
    fn request_id_uuid(id: &str) -> uuid::Uuid {
        let uuid = id
            .get(id.len().saturating_sub(36)..)
            .expect("request ID should end in a canonical UUID");
        uuid::Uuid::parse_str(uuid).expect("request ID suffix should be a parseable UUID")
    }

    #[test]
    fn request_id_preserves_namespace_and_contains_uuid_v7() {
        let id = generate_request_id("opaque-client");
        assert!(
            id.starts_with("opaque-client-"),
            "request ID should preserve its client namespace: {id}"
        );

        let uuid = request_id_uuid(&id);
        assert_eq!(
            uuid.get_version(),
            Some(uuid::Version::SortRand),
            "request ID suffix must be UUID v7: {id}"
        );

        let before_uuid = id
            .strip_suffix(&uuid.to_string())
            .expect("canonical UUID should be the request ID suffix");
        let millis = before_uuid
            .strip_prefix("opaque-client-")
            .and_then(|value| value.strip_suffix('-'))
            .expect("request ID should retain its explicit millisecond field");
        assert!(
            millis.parse::<u64>().is_ok(),
            "millisecond field should remain directly inspectable: {id}"
        );
    }

    #[test]
    fn request_ids_are_unique_v7_and_ordered_in_a_tight_loop() {
        const SAMPLE_SIZE: usize = 100_000;

        let mut seen = HashSet::with_capacity(SAMPLE_SIZE);
        let mut previous: Option<String> = None;
        for _ in 0..SAMPLE_SIZE {
            let id = generate_request_id("load-client");
            assert!(
                seen.insert(id.clone()),
                "request ID collided in {SAMPLE_SIZE}-item tight loop: {id}"
            );
            assert_eq!(
                request_id_uuid(&id).get_version(),
                Some(uuid::Version::SortRand),
                "request ID was not UUID v7: {id}"
            );
            if let Some(previous) = &previous {
                assert!(
                    id > *previous,
                    "request IDs must follow generation order: {previous} then {id}"
                );
            }
            previous = Some(id);
        }
